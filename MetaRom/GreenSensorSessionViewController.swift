//
//  GreenSensorSessionViewController.swift
//  MetaRom
//
...
protocol GreenSensorSessionDelegate: class {
    func didFinish(lower: MetaWear)
}

class GreenSensorSessionViewController: UIViewController {
    @IBOutlet var nextButton: UIButton!
    @IBOutlet var resetButton: UIButton!
    @IBOutlet var searchingLabel: UILabel!
    @IBOutlet var activityIndicator: UIActivityIndicatorView!
    
    weak var delegate: GreenSensorSessionDelegate?
    var scannerModel: ScannerModel!
    var alreadyChosenDevice: MetaWear!
    ...
    func updateDetectedSensors() {
        DispatchQueue.main.async {
            ...
            if sortedDevices.count > 0 && self.chosenDevice == nil && sortedDevices.first?.1 != self.alreadyChosenDevice {
                ...
                self.chosenDevice!.connectAndSetup().continueWith(.mainThread) { t in
                    if let error = t.error {
                        ...
                        self.chosenDevice!.flashLED(color: .green, intensity: 0.75, _repeat: 0xFF, onTime: 200, period: 1500)
                    }
    ...
    @IBAction func nextPressed(_ sender: Any) {
        ...
        scannerModel.isScanning = false
        self.nextButton.isEnabled = false
        self.delegate?.didFinish(lower: self.chosenDevice!)
    }
    
}

extension GreenSensorSessionViewController: ScannerModelDelegate {
